...
export function SavedSystems({ refreshKey = 0 }: { refreshKey?: number }) {
  ...
  if (!user) return <div className="card p-5 text-muted sm:p-6">{t('Dashboard', 'Sign in to see saved scenarios and history.')}</div>;
  if (loading) return <div className="card p-5 text-muted sm:p-6">{t('Common', 'Loading scenarios...')}</div>;
  if (!systems.length) return <div className="card p-5 text-muted sm:p-6">{t('Dashboard', 'No saved systems yet. Run an assessment and click "Save".')}</div>;
          ...
          <div key={system.id} className="card min-w-0 p-4 sm:p-5">
            ...
            <div className="flex justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="text-sm text-muted">
                  {new Date(system.createdAt).toLocaleDateString(dateLocale, { day: 'numeric', month: 'long', year: 'numeric' })}
                </p>
              </div>
              <button
            ...
            <div className="mt-3 grid grid-cols-3 gap-2 text-sm">
              ...
              <div className="rounded-xl bg-amber-100 p-2 text-center">
                ...
                <div className="text-xs text-muted">{t('Dashboard', 'type')}</div>
              </div>
            </div>

            {/* Advice excerpt */}
            <p className="mt-3 line-clamp-2 text-sm leading-6 text-muted">{typeof system.advice === 'string' ? system.advice : ''}</p>
            ...
            >
              <Eye size={16} /> {t('Dashboard', 'View full analysis')}
            </button>
          </div>
        ))}
      </div>
