...
export function DayNightChart({ result }: { result: RecommendationResult }) {
  ...
  const localizedData = result.chartData.map((item) => ({
    ...item,
    name: item.name === 'Day' ? t('Results', 'Day') : item.name === 'Evening' ? t('Results', 'Evening') : item.name,
  }));
        ...
        {width > 0 && (
          <ResponsiveContainer width={width} height={300}>
            <PieChart margin={{ top: 10, right: 10, bottom: 10, left: 10 }}>
              <Pie data={localizedData} dataKey="value" nameKey="name" cx="50%" cy="45%" innerRadius={width < 400 ? 30 : 50} outerRadius={width < 400 ? 60 : 90} paddingAngle={2}>
                <Cell fill="#16A34A" />
                <Cell fill="#F59E0B" />
              </Pie>
