    ...
    <div className="mobile-card border border-white/12 bg-white/[0.055] p-4 shadow-card backdrop-blur-xl sm:p-5">
      ...
      <div className="mt-4 grid gap-3">
        {result.topConsumers.map((item) => (
          <div key={item.name} className="grid gap-2 rounded-[1.2rem] border border-white/10 bg-white/8 p-3 sm:grid-cols-[1fr_auto] sm:items-center">
            <div>
            ...
            <div className="flex flex-wrap gap-2 text-sm font-black text-white">
              <span className="rounded-xl bg-navy/60 px-3 py-2">{item.monthlyKwh} kWh/мес.</span>
              <span className="rounded-xl bg-mint/15 px-3 py-2 text-mint">{item.percent}%</span>
            </div>
          </div>
        ))}
      </div>
