...
export function AppliancePresetCard({ preset, selected, onToggle }: { preset: AppliancePreset; selected: boolean; onToggle: (appliance: ApplianceInput) => void }) {
  ...
  return (
    <motion.button type="button" whileHover={{ y: -3 }} whileTap={{ scale: 0.985 }} onClick={() => onToggle({ ...preset })} className={`relative min-h-40 overflow-hidden rounded-[1.2rem] border p-4 text-left transition ${selected ? 'border-mint bg-mint/14 shadow-glow' : 'border-white/12 bg-white/[0.055] hover:border-cyan/45'}`}>
      <div className="absolute right-0 top-0 h-24 w-24 rounded-full bg-cyan/10 blur-2xl" />
      <div className="relative flex items-start gap-3">
        <span className={`grid h-11 w-11 shrink-0 place-items-center rounded-2xl ${selected ? 'bg-mint text-navy' : 'bg-white/8 text-cyan'}`}>
      ...
      {badges.length > 0 && <div className="relative mt-3 flex flex-wrap gap-1.5">{badges.slice(0, 3).map((badge) => <span key={badge} className="rounded-full bg-white/8 px-2 py-1 text-[11px] font-bold text-slate-300">{badge}</span>)}</div>}
    </motion.button>
  );
}
